...
public class DetectButtonTouch implements TouchController {
  ...
  public boolean processTouchEvent(TouchEvent event, Vector2 touchPoint, int pointerID) {
    ...
    if (pointerID == BUTTON_POINTER && TouchButton.isDraggedOutsideButton(buttonBounds,
        event.type)) {
      BUTTON_POINTER = -1;
    ...
    if (pointerID == BUTTON_POINTER && TouchButton.isDragged(event.type, startTouchPoint,
        touchPoint)) {
      BUTTON_POINTER = -1;
      button.buttonReleased();
      processed = true;
